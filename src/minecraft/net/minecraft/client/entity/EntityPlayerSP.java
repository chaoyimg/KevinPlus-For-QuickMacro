    ...
    public void onUpdateWalkingPlayer() {
        try {
            ...
            if (this.isCurrentViewEntity())
            {
                ...
                {
                    if (moved && rotated) {this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(posX, posY, posZ, yaw, pitch, isOnGround));}
                    else if (moved) {this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY, posZ, isOnGround));}
                    else if (rotated) {this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(yaw, pitch, isOnGround));}
                    else {this.sendQueue.addToSendQueue(new C03PacketPlayer(isOnGround));}
                    final CTPMender ctpMender = (CTPMender) KevinClient.moduleManager.getModule(CTPMender.class);
                    if (ctpMender.getState()) {ctpMender.processPackets();}
                }
                else
                {
