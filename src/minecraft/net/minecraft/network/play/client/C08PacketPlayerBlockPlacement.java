...
public class C08PacketPlayerBlockPlacement implements Packet<INetHandlerPlayServer>
{
    ...
    public void readPacketData(PacketBuffer buf) throws IOException
    {
        ...
        this.facingX = highVerFix.getState() ? (float)buf.readUnsignedByte() :(float)buf.readUnsignedByte() / 16.0F;
        this.facingY = highVerFix.getState() ? (float)buf.readUnsignedByte() :(float)buf.readUnsignedByte() / 16.0F;
        this.facingZ = highVerFix.getState() ? (float)buf.readUnsignedByte() :(float)buf.readUnsignedByte() / 16.0F;
    }
    ...
    public void writePacketData(PacketBuffer buf) throws IOException
    {
        ...
        HighVerFix highVerFix = KevinClient.moduleManager.getModule(HighVerFix.class);
        buf.writeByte((int)(highVerFix.getState() ? this.facingX : this.facingX * 16.0F));
        buf.writeByte((int)(highVerFix.getState() ? this.facingY : this.facingY * 16.0F));
        buf.writeByte((int)(highVerFix.getState() ? this.facingZ : this.facingZ * 16.0F));
    }
