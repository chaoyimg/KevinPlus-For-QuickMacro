...
class KillAura : Module("Aura","Automatically attacks targets around you.", Keyboard.KEY_R, ModuleCategory.COMBAT) {
    /**
     * OPTIONS
     */
    ...
    private val swinghurtTimeValue = IntegerValue("SwingHurtTime", 10, 0, 10)
    ...
    // Bypass
    private val swingValue = ListValue("SwingMode", arrayOf("Normal", "OnlyWhenNoHurt", "Packet","CustomWhenHurt", "OFF"), "Normal")
    ...
    = ListValue("ExtraBlockTiming", arrayOf("NoExtra","AfterAttack", "Pre", "Post", "Update", "Pre&Post", "Update&Pre", "Update&Post", "Update&Pre&Post"), "NoExtra")
    ...
    private val switchstop = BooleanValue("SwitchStopBlock", true) { autoBlockValue notEqual  "Off" }
...
//                GL11.glDisable(GL11.GL_BLEND)
    ...
    private fun doSwing() {
        when(swingValue.get()) {
            ...
            "CustomWhenHurt" -> {
                if (currentTarget == null || currentTarget !is EntityLivingBase || (currentTarget as EntityLivingBase).hurtTime <= swinghurtTimeValue.get())
                    mc.thePlayer.swingItem()
                else
                    mc.netHandler.addToSendQueue(C0APacketAnimation())
            }
    ...
    private fun runAttack() {
        ...
        if (!hitable || failHit || range > maxRange) {
            ...
            if (extraBlockTimingValue equal "AfterAttack" ){
                runBlock()
            }
 ...
 **/
    ...
    private fun stopBlocking() {
        if (blockingStatus) {
            if (autoBlockValue notEqual "Fake") {if (!switchstop.get()) {mc.netHandler.addToSendQueue(C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN))}else{
